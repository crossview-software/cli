...
describe("Cli.processArgs", function() {
	it("should process arguments and call the correct handlers and flags", function() {
		...
		expect(
			getNewProcessableCli(true).processArgs(veryRatherGoodDayArgs)
		).to.equal("have a very rather good day")
		expect(
			getNewProcessableCli(true).processArgs(ratherGoodDayArgs)
		).to.equal("have a rather good day")
		expect(
			getNewProcessableCli(true).processArgs(trulyQuiteBadDayArgs)
		).to.equal("have a truly quite bad day")
		expect(
			getNewProcessableCli(true).processArgs(quiteBadDayArgs)
		).to.equal("have a quite bad day")
		expect(
			getNewProcessableCli(true).processArgs(quiteTrulyAwfulBadDayArgs)
		).to.equal("have a quite truly awful bad day")
	})
	...
	it("should set args equal to the third process.argv onward if args is null", function() {
		...
		const cli = getNewTestCli(true).registerHandler()
	...
	it("should throw an error if an invalid flag is passed to processArgs", function() {
		...
		expect(() => getNewProcessableCli(true).processArgs(testArgs)).to.throw(
			Error,
			"Invalid flag: -x"
		)
	})
	...
	it("should call the handler and return the Cli object if shouldExecuteHandlers is true but shouldReturnHandler is false", function() {
		...
		const cli = getNewTestCli().registerHandler(opts => {
			output.handlerIsCalled = true
		})
	...
	it("should just return the Cli object if shouldExecuteHandlers is false and the help flag is not set", function() {
		...
		const cli = getNewTestCli(false, false).registerHandler(opts => {
			output.handlerIsCalled = true
		})
	...
	it("should call the handler for each selected option if no command handler is set", function() {
		...
		const cli = getNewTestCli()
			.registerFlag(
				"scrantonicity",
				"s",
				"The best Police cover band in Scranton",
				() => (output.option1HandlerIsCalled = true)
			)
			.registerFlag(
				"jokersandtokers",
				"j",
				"The best Steve Miller cover band in Scranton",
				() => (output.option2HandlerIsCalled = true)
			)
			.registerFlag(
				"dundermifflinwarehouseband",
				"d",
				"This band didn't let Kevin play",
				() => true
			)
...
describe("Cli.registerCommand", function() {
	...
	it("should throw an error when registerCommand is called with two commands with the same Cli.name", function() {
		const cli = getNewTestCli().registerCommand(getNewTestCli())

		expect(() => cli.registerCommand(getNewTestCli())).to.throw(
			Error,
			"Command testCli has already been defined"
		)
	})
})
...
describe("Cli.registerFlag", function() {
	it("should register a flag on the Cli object when registerFlag is called", function() {
		const cli = getNewTestCli()
			.registerFlag("boss", "b", "Test Flag", () =>
				console.log("Welcome to Scranton")
			)
			.registerFlag("jim", "j", "Farewell Flag", () =>
				console.log("Goodbye Scranton")
			)
...
describe("Cli.registerHandler", function() {
	it("should register a function handler when registerHandler is called", function() {
		const cli = getNewTestCli().registerHandler(() => 42)
...
describe("Cli.findCommand", function() {
	it("should return a Cli from Cli.commands when findCommand is called", function() {
		const cli = getNewTestCli().registerCommand(new Cli("testing2"))

		expect(cli.findCommand("testing2"))
			.to.be.an("object")
			.that.has.property("name", "testing2")
	})
})
...
describe("Cli.findFlag", function() {
	it("should return the correct flag when findFlag is called", function() {
		...
		expect(cli.findFlag("--test1"))
			.to.be.an("object")
			.that.has.property("flag", "test1")
		expect(cli.findFlag("--test2"))
			.to.be.an("object")
			.that.has.property("flag", "test2")
	})

	it("should return null if the findFlag arg length is less than 3, but is also not a short flag", function() {
		const cli = getNewTestCli().registerFlag(
			"test1",
			"1",
			"Test Flag 1",
			null
	...
	it("should call findShortFlag if a short flag is supplied to findFlag", function() {
		...
		expect(cli.findFlag("-1"))
			.to.be.an("object")
			.that.has.property("flag", "test1")
		expect(cli.findFlag("-2"))
			.to.be.an("object")
			.that.has.property("flag", "test2")
	})
})
...
describe("Cli.findShortFlag", function() {
	it("should return the correct flag when findShortFlag is called", function() {
		...
		expect(cli.findShortFlag("-1"))
			.to.be.an("object")
			.that.has.property("flag", "test1")
		expect(cli.findShortFlag("-2"))
			.to.be.an("object")
			.that.has.property("flag", "test2")
	})
 ...
 * @returns {Cli}
 */
function getNewTestCli(
	shouldReturnHandler = false,
	shouldExecuteHandlers = true
) {
	return new Cli(
		"testCli",
		"Testing the CLI",
		shouldReturnHandler,
		shouldExecuteHandlers
	)
}
...
function getGoodDayWisher(
	...
	return new Cli(
		...
		.registerFlag(
			"very",
			"v",
			"Wishes you a very good day",
			msg => `very ${msg}`
		)
		.registerFlag(
			"rather",
			"r",
			"Wishes you a rather good day",
			msg => `rather ${msg}`
		)
 ...
 * @returns {Cli}
 */
function getBadDayWisher(
	shouldReturnHandler = false,
	shouldExecuteHandlers = true
) {
	return new Cli(
		"bad",
		"Wishes you a bad day",
		shouldReturnHandler,
		shouldExecuteHandlers
	)
		.registerFlag("quite", "q", "Wishes you a quite bad day", function(
			msg
		) {
			let prepended = ""
